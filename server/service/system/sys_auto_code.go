...
import (
	...
	ast2 "github.com/pauljohn21/cms-gva/server/utils/ast"

	cp "github.com/otiai10/copy"
	"github.com/pauljohn21/cms-gva/server/resource/autocode_template/subcontract"
	"go.uber.org/zap"
...
func Init(Package string) {
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			...
			importCodeF:  "github.com/pauljohn21/cms-gva/server/%s/%s",
			packageNameF: "%s",
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for _, s := range webTemp {
		err := os.MkdirAll(filepath.Join(s, packageName), 0o755)
		if err != nil {
			return err
		}
	}
...
func installation(path string, formPath string, toPath string) error {
	...
	form := filepath.ToSlash(global.GVA_CONFIG.AutoCode.Root + formPath + "/" + path)
	to := filepath.ToSlash(global.GVA_CONFIG.AutoCode.Root + toPath + "/plugin/")
