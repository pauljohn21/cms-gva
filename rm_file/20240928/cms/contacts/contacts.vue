<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="地址" prop="addres" width="120" />
        <el-table-column align="left" label="邮箱" prop="email" width="120" />

        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateContactsFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="电话:"  prop="phone" >
              <el-input v-model.number="formData.phone" :clearable="true" placeholder="请输入电话" />
            </el-form-item>
...
// 更新行
const updateContactsFunc = async(row) => {
    const res = await findContacts({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteContactsFunc = async (row) => {
    const res = await deleteContacts({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
