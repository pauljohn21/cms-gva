  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="保单号" prop="code" width="120" />
          <el-table-column align="left" label="申请人" prop="applicant" width="120" />

          <el-table-column align="left" label="被申请人" prop="respondent" width="120" />

          <el-table-column align="left" label="法院" prop="court" width="120" />
        <el-table-column align="left" label="保费" prop="coverage" width="120" />
        <el-table-column align="left" label="保额" prop="coverageAll" width="120" />
          <el-table-column align="left" label="开始时间" prop="startCreatedAt" width="180">
            <template #default="scope">{{ formatDate(scope.row.startCreatedAt) }}</template>
          </el-table-column>
         <el-table-column align="left" label="结束时间" prop="endCreatedAt" width="180">
            <template #default="scope">{{ formatDate(scope.row.endCreatedAt) }}</template>
         </el-table-column>
        <el-table-column align="left" label="保险信息" prop="info" width="120" />
<!--        <el-table-column align="left" label="投保人" prop="policyholder" width="120" />-->
        <el-table-column align="left" label="签署状态" prop="signStatus" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.signStatus,signStatusOptions) }}
            </template>
        </el-table-column>

        <el-table-column align="left" label="文件下载" prop="templateFileUrl" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateMeLetterFunc(scope.row)">变更</el-button>
...
<!--              <el-input v-model="formData.applicant" :clearable="true"  placeholder="请输入申请人" />-->
              <el-select v-model="formData.policyholder" placeholder="请选择申请人" style="width:100%" :clearable="true" >
                <el-option v-for="(item,key) in contactsOptions" :key="key" :label="item.name" :value="item.name" />
              </el-select>
            </el-form-item>
            <el-form-item label="投保人:"  prop="policyholder" >
              <el-select v-model="formData.applicant" placeholder="请选择申请人" style="width:100%" :clearable="true" >
                <el-option v-for="(item,key) in applicantOptions" :key="key" :label="item.company" :value="item.company" />
              </el-select>
<!--              <el-input v-model="formData.policyholder" :clearable="true"  placeholder="请输入投保人" />-->
            ...
            <el-form-item label="法院:"  prop="court" >
              <el-select v-model="formData.court" placeholder="请选择申请人" style="width:100%" :clearable="true" >
                <el-option v-for="(item,key) in courtOptions" :key="key" :label="item.name" :value="item.name" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="保额:"  prop="coverageAll" >
              ...
              <el-form-item label="开始时间:"  prop="startCreatedAt" >
                <el-date-picker v-model="formData.startCreatedAt" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
              </el-form-item>
            </el-form-item>
            <el-form-item label="结束时间:"  prop="endCreatedAt" >
              <el-date-picker v-model="formData.endCreatedAt" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
            </el-form-item>
...
const getContactsData = async() => {
  const table = await getContactsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
    contactsOptions.value = table.data.list
  }
}
const getApplicantData = async() => {
  const table = await getApplicantList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
    applicantOptions.value = table.data.list
  }
}
const getCourtData = async() => {
  const table = await getCourtList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const getTableData = async() => {
  const table = await getMeLetterList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
